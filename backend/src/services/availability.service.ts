...
export const getUserAvailabilityForPublicService = async (eventId: string) => {
...
const generateAvailableTimeSlots = (
  ...
  if (isToday) {
    slotStartTime = addMinutes(now, 1);
  }
...
const isSlotAvailable = (
  ...
  for (const meeting of meetings) {
    if (slotStart < meeting.endTime && meeting.startTime < slotEnd) {
      return false;
    }
  }
  return true;

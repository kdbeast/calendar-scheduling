...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex h-12.5 items-center gap-1 justify-start">
          <div
          ...
          {state !== "collapsed" && (
            <div className="grid flex-1 text-left text-2xl leading-tight ml-px">
              <h2 className="truncate font-medium">Slotify</h2>
            </div>
          )}
          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              ...
              >
                <Link
                  to={item.url}
                  className="text-[16px]! p-[12px_8px_12px_16px]! min-h-12! rounded-xl!
                  font-semibold!"
                >
